...
export default function TestimonialsCarousel({ testimonials }) {
  ...
  // Dots calculation: we want one dot per sliding step up to the maximum offset
      ...
      <div className="relative overflow-hidden w-full py-4">
        ...
        >
          {testimonials.map((t) => (
            ...
            >
              <div className="w-full flex flex-col justify-between rounded-3xl bg-white/5 p-8 backdrop-blur border border-white/10 transition-all duration-300 hover:bg-white/10 hover:border-white/20 hover:-translate-y-1">
                <div>
                  <Quote className="h-8 w-8 text-blue-300/20 mb-4" />
                ...
                <div className="mt-8 flex items-center gap-4 pt-6 border-t border-white/10">
                  {t.image && (
                    <div className="relative h-12 w-12 rounded-full overflow-hidden border border-blue-400 shrink-0">
                      <Image
                        src={t.image}
                        alt={t.name}
                        fill
