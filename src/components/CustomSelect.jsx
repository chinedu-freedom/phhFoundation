...
export default function CustomSelect({ value, onChange, options, placeholder = "Select...", className = "w-full md:w-48", icon }) {
  ...
  return (
    <div className={`relative inline-block ${className}`} ref={containerRef}>
      <button
        ...
        className={`relative flex h-11 w-full items-center justify-between gap-2 rounded-md border border-zinc-200 bg-white py-2.5 text-sm text-zinc-900 focus:border-blue-500 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-white cursor-pointer transition-colors ${
          icon ? "pl-10 pr-4" : "px-4"
        }`}
      >
        {icon && <div className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 flex items-center justify-center text-zinc-400">{icon}</div>}
